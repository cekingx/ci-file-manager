...
<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
    <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-2">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Jaspel Nakes/Non Nakes</h5>
            <!--end::Page Title-->
            <!--end::Actions-->
        </div>
        <!--end::Info-->
            ...
            <div class="card card-custom col-xl-12">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">Jaspel Nakes
                            <span class="d-block text-muted pt-2 font-size-sm">Data jaspel yang pernah dibuat</span></h3>
                    </div>
...
<script>
    ...
    $('.btnTambah').click(function() {
        ...
        $('.data_list').html('<div data-repeater-item="" class="form-group row align-items-center indikator_list" style="">' +
            ...
            '<input type="number" class="form-control biaya" placeholder="Rp.">' +
            '<div class="d-md-none mb-2"></div>' +
            '</div>' +
            '<div class="col-md-2">' +
    ...
    $('.btnUpdate').click(function() {
        if ($('.dokter_list_edit').val() == '') {
            ...
            items = [];
            $('.indikator_list_edit').each(function() {
    ...
    function getDokter() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('jaspel/get_nakes') ?>',
            dataType: 'json',
            success: function(data) {
                let html = "<option value=''>Pilih Nakes/Non Nakes</option>";
        ...
        var demo = function() {
            ...
            var datatable = $('#kt_datatable').KTDatatable({
                ...
                data: {
                    ...
                    source: {
                        read: {
                            url: '<?php echo base_url('jaspel/get_list_nakes') ?>',
                ...
                columns: [{
                    ...
                    template: function(row) {
                        return '\
							<a href="javascript:;" data-id="' + row.jaspel_id + '" data-bulan="' + row.jaspel_bulan + '" data-tahun="' + row.jaspel_tahun + '" data-dokter="' + row.nakes_kode + '" class="btn btn-sm btn-clean btn-icon btnEdit" title="Edit details">\
                    ...
                    success: function(data) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            html += '<div data-repeater-item="" class="form-group row align-items-center indikator_list_edit" style="">' +
                                ...
                                '<div class="col-md-5">' +
                                '<label>Biaya (Rp.):</label>' +
                                '<input type="number" class="form-control biaya_edit" placeholder="Rp." value="' + data[i].jaspel_detail_biaya + '">' +
                                '<div class="d-md-none mb-2"></div>' +
                                '</div>' +
                                '<div class="col-md-2">' +
                        ...
                        $('.add_edit').click(function() {
                            html = '<div data-repeater-item="" class="form-group row align-items-center indikator_list_edit" style="">' +
                                ...
                                '<div class="col-md-5">' +
                                '<label>Biaya (Rp.):</label>' +
                                '<input type="number" class="form-control biaya_edit" placeholder="Rp.">' +
                                '<div class="d-md-none mb-2"></div>' +
                                '</div>' +
                                '<div class="col-md-2">' +
