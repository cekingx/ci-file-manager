        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form form_add">
                    <div class="card-body">
                        <input type="hidden" class="form-control spesialis_id">
                        <div class="form-group">
                            <label>Spesialis</label><br>
                            <input type="text" class="form-control spesialis" placeholder="Spesialis">
                        </div>
                    </div>
                </form>
            </div>
...
<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
    <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-2">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Master Data Spesialis</h5>
            <!--end::Page Title-->
            <!--end::Actions-->
        </div>
        <!--end::Info-->
            ...
            <div class="card card-custom col-xl-12">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">Master Data Spesialis
                            <span class="d-block text-muted pt-2 font-size-sm">Manajemen Master Data Spesialis</span></h3>
                    </div>
...
<script>
    $('.btnTambah').click(function() {
        ...
        $('#modalTambahTitle').html('Tambah Spesialis');
        $('.btnUpdate').fadeOut(1);
        $('.btnSave').fadeIn(1);
        $('#modalTambah').modal();
    });
    ...
    $('.btnSave').click(function() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('Master_spesialis/add') ?>',
            dataType: 'json',
            data: {
    ...
    $('.btnUpdate').click(function() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('Master_spesialis/update') ?>',
            dataType: 'json',
            data: {
                spesialis_id: $('.spesialis_id').val(),
                spesialis: $('.spesialis').val(),
            },
            success: function(data) {
                console.log(data)
        ...
        var demo = function() {
            ...
            var datatable = $('#kt_datatable').KTDatatable({
                ...
                data: {
                    ...
                    source: {
                        read: {
                            url: '<?php echo base_url('Master_spesialis/get_list_spesialis') ?>',
                ...
                columns: [{
                    ...
                    template: function(row) {
                        return '\
							<a href="javascript:;" data-id="' + row.spesialis_id + '" data-nama="' + row.spesialis + '" class="btn btn-sm btn-clean btn-icon btnEdit" title="Edit details">\
								<i class="la la-edit"></i>\
							</a>\
							<a href="javascript:;" data-id="' + row.spesialis_id + '" class="btn btn-sm btn-clean btn-icon btnDelete" title="Delete">\
                ...
                bootbox.confirm({
                    ...
                    callback: function(result) {
                        if (result) {
                            $.ajax({
                                type: 'POST',
                                url: '<?php echo base_url('Master_spesialis/delete') ?>',
                                data: {
                                    spesialis_id: id,
                                },
                                dataType: 'json',
            ...
            $(document).on("click", ".btnEdit", function() {
                ...
                $('#modalTambahTitle').html('Update Spesialis');
                $('.spesialis_id').val(id);
                $('.spesialis').val(nama);
