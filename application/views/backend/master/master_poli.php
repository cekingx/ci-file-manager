        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form form_add">
                    <div class="card-body">
                        <input type="hidden" class="form-control poli_id">
                        <div class="form-group">
                            <label>Nama Unit/Poli</label><br>
                            <input type="text" class="form-control poli_nama" placeholder="Nama Poli">
                        </div>
                    </div>
                </form>
            </div>
...
<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
    <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-2">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Master Data Unit/Poli</h5>
            <!--end::Page Title-->
            <!--end::Actions-->
        </div>
        <!--end::Info-->
            ...
            <div class="card card-custom col-xl-12">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">Master Data Unit/Poli
                            <span class="d-block text-muted pt-2 font-size-sm">Manajemen Master Data Unit/Poli</span></h3>
                    </div>
...
<script>
    ...
    $('.btnTambah').click(function() {
        ...
        $('#modalTambahTitle').html('Tambah Unit/Poli');
        $('.btnUpdate').fadeOut(1);
        $('.btnSave').fadeIn(1);
        $('#modalTambah').modal();
    });
    ...
    $('.btnSave').click(function() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('Master_poli/add') ?>',
            dataType: 'json',
            data: {
                poli_nama: $('.poli_nama').val(),
            },
    ...
    $('.btnUpdate').click(function() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('Master_poli/update') ?>',
            dataType: 'json',
            data: {
                poli_id: $('.poli_id').val(),
                poli_nama: $('.poli_nama').val(),
        ...
        var demo = function() {
            ...
            var datatable = $('#kt_datatable').KTDatatable({
                ...
                data: {
                    ...
                    source: {
                        read: {
                            url: '<?php echo base_url('Master_poli/get_list_poli') ?>',
                ...
                columns: [{
                    ...
                    template: function(row) {
                        return '\
							<a href="javascript:;" data-id="' + row.poli_id + '" data-nama="' + row.poli_nama + '" class="btn btn-sm btn-clean btn-icon btnEdit" title="Edit details">\
								<i class="la la-edit"></i>\
							</a>\
							<a href="javascript:;" data-id="' + row.poli_id + '" class="btn btn-sm btn-clean btn-icon btnDelete" title="Delete">\
                ...
                bootbox.confirm({
                    ...
                    callback: function(result) {
                        if (result) {
                            $.ajax({
                                type: 'POST',
                                url: '<?php echo base_url('Master_poli/delete') ?>',
                                data: {
                                    poli_id: id,
                                },
                                dataType: 'json',
                                success: function(data) {
                                    if (data) {
                                        bootbox.alert({
                                            message: "Unit/Poli berhasil dihapus",
                                            backdrop: true,
                                            size: 'small',
                                            callback: function(){
                                                location.reload();
            ...
            $(document).on("click", ".btnEdit", function() {
                ...
                $('#modalTambahTitle').html('Update Unit/Poli');
                $('.poli_id').val(id);
                $('.poli_nama').val(nama);
